...
class LocalCameraController:
    ...
    async def capture_latest_image(
        ...
        camera_service: CameraService = Provide[
            Go2MiddleLayerContainer.local_camera_service
        ],
        ...
        """
        reason = service_registry.get_unavailable_reason("local_camera")
        if reason:
            return Response(
                success=False,
                message=f"Local camera not available: {reason}",
                data=None,
                code=503,
            )
        ...
        if data is None:
            return Response(
                success=False,
                message=f"Failed to capture image from buffer. Message: {message}",
                data=None,
                code=400,
            )
        ...
        return Response(
            success=True,
            message=f"Image captured successfully from buffer. Message: {message}",
            data=image_data,
            code=200,
        )
